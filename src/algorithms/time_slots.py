"""Time-based edge occupancy tracking - Numba compatible."""
...
def count_vehicles_at_time(
    ...
    Xe được tính nếu: start_time <= query_time < end_time
    """
    count = 0
...
def count_vehicles_in_interval(
    ...
    Xe overlap nếu: NOT (end <= interval_start OR start >= interval_end)
    Tức là: start < interval_end AND end > interval_start
    """
    count = 0
    max_slots = edge_time_slots.shape[1]
...
def add_vehicle_to_edge(
    ...
    """
    Thêm xe vào cạnh với thời gian vào/ra.
    
    Returns: slot index nếu thành công, -1 nếu hết slot
    """
    max_slots = edge_time_slots.shape[1]
    
    for slot in range(max_slots):
        if edge_time_slots[edge_idx, slot, 0] < 0.0:  # slot trống
...
def get_max_load_in_interval(
    ...
    """
    Tìm load cao nhất trên cạnh trong khoảng thời gian.
    
    Sampling theo time_step để tìm max.
    """
    max_load = 0
    t = interval_start
...
def compute_path_times(
    ...
    """
    Tính thời gian vào/ra của xe trên từng cạnh trong path.
    
    Returns:
    - edge_indices: (max_path_len,) index của các cạnh đi qua, -1 nếu hết
    - entry_times: (max_path_len,) thời điểm vào mỗi cạnh
    - exit_times: (max_path_len,) thời điểm ra mỗi cạnh
    - n_edges_in_path: số cạnh thực sự
    """
...
def compute_time_penalty_for_path(
    ...
    """
    Tính cost (travel + time-based penalty) nếu thêm xe vào path.
    
    Penalty tính tại thời điểm xe VÀO mỗi cạnh:
    - Đếm số xe đang ở cạnh đó
    - Nếu (current_load + 1) > bandwidth → penalty
    """
    if base_cost == np.inf:
        return np.inf
    ...
    for i in range(n_edges):
        ...
        # Đếm xe đang overlap với khoảng thời gian này
...
def build_edge_load_timeline(
    ...
    """
    Xây dựng timeline load cho 1 cạnh.
    
    Returns:
    - times: (n_samples,) các thời điểm sample
    - loads: (n_samples,) load tại mỗi thời điểm
    """
...
def build_all_edges_load_timeline(
    ...
    """
    Xây dựng timeline load cho tất cả cạnh.
